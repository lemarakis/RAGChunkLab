...
def run_evaluation():
    ...
    print(f"--- Evaluation: Fixed-Size Chunking ---")
    print(f"Total questions to test: {total_questions}\n")

    for qid, question, expected_page, q_emb in qa_rows:
        # 2. Perform Vector Search (Top-5)
        # Postgres vector operator '<=>' is for cosine distance
        sql = """
            SELECT metadata->'pages' as pages, content
            FROM chunks_fixed_size
        ...
        # 3. Check if the expected page is in the retrieved chunks
        found_at_rank = 0
        combined_content = ""
        ...
        if found_at_rank > 0:
            ...
            # Optional: print(f"[V] {qid}: Rank {found_at_rank}")
        else:
    ...
    if valid_questions > 0:
        ...
        print(" Strategy: Fixed-Size (1000 chars)")
        print("="*45)
